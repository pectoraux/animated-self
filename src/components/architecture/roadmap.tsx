...
const phases: Phase[] = [
  ...
  },
  {
    ...
    badgeTone: "border-emerald-500/30 bg-emerald-500/10 text-emerald-200",
    body:
      "Real async render queue + worker behind the existing POST /api/render contract. Jobs transition queued → running → rendering → done (or failed) with progress polling every 500ms, and the finished MP4 is served from GET /api/render/{job_id}/file. The consent gate applies to async renders too — a bound character's face_hash is checked on every render request, matching the live path. DiffusionRenderer runs whatever audio-driven reenactment command you configure via DIFFUSION_RENDER_CMD (an AniPortrait-style setup, or anything honoring the same reference-image + audio-in, MP4-out contract) — there is no bundled default model, for the same reason THA3's weights aren't bundled: we can't verify or redistribute either one here. Engine tests cover the queue/job lifecycle, the consent gate, and the subprocess contract itself (using a stand-in command, not a real model). The control panel surfaces the queue as an Async Render card with status badges, a progress bar, and a Download MP4 button.",
    shipped: true,
    callout: {
      icon: "warning",
      title:
  ...
  },
  {
    ...
    badgeTone: "border-emerald-500/30 bg-emerald-500/10 text-emerald-200",
    body:
      "Async voice conversion (file in, file out) behind POST /api/voice/convert — the creator's mic audio is base64-encoded, the engine runs the configured converter on a temp file, and the converted WAV is served from GET /api/voice/{out_id}/download (in-process store; durable storage is a future concern). The consent gate applies for bound custom characters via the same _enforce_consent_gate as live sessions and async renders — converting audio to sound like someone's avatar is identity-affecting. Two real backends, same pattern as the diffusion renderer: ExternalCommandConverter runs VOICE_CONVERT_CMD (an RVC CLI or anything honoring the audio-in, audio-out contract — no model is bundled or verified), and CloudConverter does BYOK HTTPS (ElevenLabs speech-to-speech today, api_key per-request, never persisted). Selection priority is cmd > cloud > none. If neither env var is set, /api/voice/convert returns 503 — voice conversion is genuinely unavailable, not faked. Engine tests cover selection, the subprocess contract, the consent gate (locked / unrelated-token / matching-token / stock), the 503 unconfigured case, and the download endpoint. The control panel surfaces this as a Voice Conversion card with character picker, audio upload, optional BYOK API key field, and a result panel that distinguishes live-success (Download WAV), demo-simulated (disabled button + honest note), and the 503/403/502/400 error cases.",
    shipped: true,
    callout: {
      icon: "warning",
      title:
